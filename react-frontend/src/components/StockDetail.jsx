    ...
    <div className="space-y-8 pb-12 animate-in fade-in slide-in-from-right-4 duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-neutral-800 pb-6 gap-4">
        <div>
          ...
          <div className="flex items-center space-x-4 text-sm">
            ...
            <span className="text-neutral-400 font-medium">STRENGTH: <span className="text-neutral-100 uppercase tracking-widest">{analysis?.strength}</span></span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="xl:col-span-2 space-y-6">
          <div className="p-6 bg-neutral-900/50 border border-neutral-800 rounded-3xl overflow-hidden shadow-2xl">
        ...
        <div className="space-y-6">
          ...
          <div className="p-6 bg-neutral-900/50 border border-neutral-800 rounded-3xl">
            ...
            <div className="space-y-3">
              {analysis?.signals?.map((sig, i) => (
                <div key={i} className={`p-4 rounded-2xl text-xs font-bold border flex items-center ${
                    sig.includes('Bullish') ? 'bg-emerald-500/5 text-emerald-400 border-emerald-500/10' : 
                    sig.includes('Bearish') ? 'bg-red-500/5 text-red-400 border-red-500/10' :
                    'bg-neutral-800 text-neutral-400 border-neutral-700'
                }`}>
                  <div className={`w-2 h-2 rounded-full mr-3 ${sig.includes('Bullish') ? 'bg-emerald-500' : sig.includes('Bearish') ? 'bg-red-500' : 'bg-neutral-600'}`}></div>
                  {sig}
                </div>
              ))}
            </div>
          ...
          <div className="p-6 bg-neutral-900 border border-neutral-800 rounded-3xl">
            <h3 className="text-sm font-semibold text-neutral-500 mb-6 flex items-center">
              <Info size={14} className="mr-2 text-blue-500" /> INDICATOR DATA
            </h3>
            <div className="grid grid-cols-1 gap-4">
              ...
              <div className="flex justify-between items-center p-3 border-b border-neutral-800">
                <span className="text-xs text-neutral-500 font-medium tracking-tight">SMA 50</span>
                <span className="text-sm font-bold text-neutral-100">{indicators.currentSMA50?.toFixed(2) || 'N/A'}</span>
              </div>
              <div className="flex justify-between items-center p-3 border-b border-neutral-800">
                <span className="text-xs text-neutral-500 font-medium tracking-tight">SMA 200</span>
                <span className="text-sm font-bold text-neutral-100">{indicators.currentSMA200?.toFixed(2) || 'N/A'}</span>
              </div>
              <div className="flex justify-between items-center p-3">
                <span className="text-xs text-neutral-500 font-medium tracking-tight">MACD Signal</span>
                <span className="text-sm font-bold text-neutral-200">
                   {indicators.currentMACD?.signal?.toFixed(2) || '0.00'}
                </span>
              </div>
            </div>
          </div>
